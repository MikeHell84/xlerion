<footer class="bg-gray-800 border-t border-gray-700">
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="footer-columna mb-6">
                <a href="/contacto" class="footer-titulo flex items-center font-bold text-lg hover:text-teal-400 transition pb-2"><i class="fas fa-phone mr-2"></i>Contacto</a>
                <ul class="space-y-2">
                    <li class="flex items-center"><a href="https://wa.me/573208605600" target="_blank" class="flex items-center hover:text-teal-400 transition"><i class="fab fa-whatsapp mr-2"></i>WhatsApp: +57 320 860 5600</a></li>
                </ul>
                <a href="/contacto" class="footer-titulo flex items-center font-bold text-lg hover:text-teal-400 transition mt-4 pb-2"><i class="fas fa-envelope mr-2"></i>Email</a>
                <ul class="space-y-2">
                    <li><a href="mailto:contactus@xlerion.com" class="flex items-center hover:text-teal-400 transition"><i class="fas fa-at mr-2"></i>contactus@xlerion.com</a></li>
                    <li><a href="mailto:support@xlerion.com" class="flex items-center hover:text-teal-400 transition"><i class="fas fa-at mr-2"></i>support@xlerion.com</a></li>
                </ul>
            </div>
            <div class="footer-columna mb-6">
                <a href="/legal" class="footer-titulo flex items-center font-bold text-lg hover:text-teal-400 transition pb-2"><i class="fas fa-gavel mr-2"></i>Legal</a>
                <ul class="space-y-2">
                    <li><a href="/legal/privacidad" class="flex items-center hover:text-teal-400 transition"><i class="fas fa-shield-alt mr-2"></i>Políticas de Privacidad</a></li>
                    <li><a href="/legal/terminos" class="flex items-center hover:text-teal-400 transition"><i class="fas fa-file-contract mr-2"></i>Términos de Uso</a></li>
                    <li><a href="/legal/avisos" class="flex items-center hover:text-teal-400 transition"><i class="fas fa-gavel mr-2"></i>Avisos Legales</a></li>
                </ul>
            </div>
            <div class="footer-columna mb-6">
                <h3 class="footer-titulo font-bold text-lg pb-2">Redes Sociales</h3>
                <ul class="flex space-x-4 justify-center md:justify-start">
                    <li><a href="https://www.linkedin.com/company/xlerion" target="_blank" aria-label="LinkedIn"><i class="fab fa-linkedin text-2xl hover:text-teal-400 transition"></i></a></li>
                    <li><a href="https://www.indiegogo.com/es/profile/miguel_rodriguez-martinez_edb9?redirect_reason#/overview" target="_blank" aria-label="Indiegogo"><i class="fas fa-globe text-2xl hover:text-teal-400 transition"></i></a></li>
                    <li><a href="https://www.kickstarter.com/profile/xlerionstudios" target="_blank" aria-label="Kickstarter"><i class="fab fa-kickstarter-k text-2xl hover:text-teal-400 transition"></i></a></li>
                    <li><a href="https://www.patreon.com/xlerionstudios" target="_blank" aria-label="Patreon"><i class="fab fa-patreon text-2xl hover:text-teal-400 transition"></i></a></li>
                    <li><a href="https://www.instagram.com/ultimatexlerion/" target="_blank" aria-label="Instagram"><i class="fab fa-instagram text-2xl hover:text-teal-400 transition"></i></a></li>
                    <li><a href="https://www.facebook.com/xlerionultimate" target="_blank" aria-label="Facebook"><i class="fab fa-facebook-f text-2xl hover:text-teal-400 transition"></i></a></li>
                    <li><a href="https://www.behance.net/xlerionultimate" target="_blank" aria-label="Behance"><i class="fab fa-behance text-2xl hover:text-teal-400 transition"></i></a></li>
                </ul>
            </div>
        </div>
        <div class="text-center mt-4">
            <p>Copyright © Xlerion</p>
        </div>
    </div>
</footer>